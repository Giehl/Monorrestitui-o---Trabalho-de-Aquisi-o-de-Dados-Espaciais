...
x0= np.array([[Xm],[Ym],[300],[0.001],[0.001],[0.001]]) 
...
def matrizA(x0,pa_pc,x,y):  
    ...
    def IterA(coef,pa_pc,x0):
        ...
        for i in pa_pc:
            for j in coef:
                num_a = j.evalf(subs={c: (15.8893/1000), X0: x0[0][0], Y0: x0[1][0],\
...
while not stop:
    ...
    for i in pa_pc:
        for j in [x,y]:
            xi= j.evalf(subs={c: (15.8893/1000), X0: x0[0][0], Y0: x0[1][0], Z0: x0[2][0],\
                              om: x0[3][0], fi: x0[4][0], kapa: x0[5][0], X:i[1],Y:i[2],Z:i[3]})
            L_0=np.append(L_0,xi)
        L00=np.matrix(L_0)
        L0=np.transpose(L00)
    ...
    if it != 1: 
        if ((np.amax(abs(X_c))) <= threshold) or (it ==10):
            stop = True
        else:
            x0 = Xa    
    it = it+1
...
# Determinação de x,y (espaço imagem) a partir dos POEs ajustados--------------
Xa=np.array(Xa)
pv_calc=[]
for i in pa_pv:
    for j in [x,y]:
        xi= j.evalf(subs={c: (15.8893/1000), X0: Xa[0][0], Y0: Xa[1][0], Z0: Xa[2][0],\
                          om: Xa[3][0], fi: Xa[4][0], kapa: Xa[5][0], X:i[1],Y:i[2],Z:i[3]})
        pv_calc=np.append(pv_calc,xi)
...
# Diferença entre os pontos de verificação calculados e observados no espaço imagem
print()
print()
print('Diferença entre pv calculados e observados no espaço imagem')
print()
dif_entre_pv_imag=cl_pv_to_xy-pv_calc
print(dif_entre_pv_imag)
...
def Col_inv_sub(Xa,cl_pv_to_x,cl_pv_to_y,pa_pv):
    ...
    for i in dd:
        for l in [x_inv,y_inv]:
            num_aa= l.evalf(subs={c: (15.8893/1000), X0: Xa[0][0], Y0: Xa[1][0], Z0: Xa[2][0],\
...
print()
dif_entre_pv_objeto=xy_inv-pa_pv_to_xy       
